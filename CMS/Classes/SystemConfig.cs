...
public class SystemConfig
{
    ...
    {
        return System.Configuration.ConfigurationManager.ConnectionStrings["CMSDataBaseConnectionString"].ConnectionString;
    }
    ...
    private void SetSysData()
    {
        if (loadDB(ref dtConfig) == 1)
        {
            for (int i = 0; i < dtConfig.Rows.Count; i++)
            {
                ...
                {
                    Name = dtConfig.Rows[i]["Short"].ToString().ToLower(),
                    Value = dtConfig.Rows[i]["ShortValue"].ToString()
                });
            }
        }
    }
    ...
    {
        string Command = "select Short,ShortValue from settingValue where Hidden=0";
